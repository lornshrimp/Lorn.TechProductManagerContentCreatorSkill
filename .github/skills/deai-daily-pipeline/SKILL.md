...
3. **文中配图规划（必填，每平台 ≥1 张，按技能配图数量规则）**：每张配图必须包含 **①锚文本定位**（在正文中唯一出现的 5-15 字短句，说明插入位置）+ **②配图 AI 提示词**（英文三段式）。按各技能「文中配图规划」章节的信号规则判断插入位置（关键数据/抽象概念/金句/长段落/对比论证），2500-3500 字正文建议 2-3 张（含封面）。输出在文件末尾 `[配图1]/[配图2]...` 标注下，并附 `## 配图搜索替换指令` 供发布时定位。
